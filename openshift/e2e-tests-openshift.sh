...
function run_go_e2e_tests() {
  ...
  go test -run="^/(?!TestKanikoTaskRun)(.*)" -v -failfast -count=1 -tags=e2e -ldflags '-X github.com/tektoncd/pipeline/test.missingKoFatal=false' ./test -timeout=20m --kubeconfig $KUBECONFIG || return 1
}
